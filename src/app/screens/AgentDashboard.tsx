    ...
    <div className="p-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex gap-4 w-full sm:w-auto">
          <div className="relative w-full sm:w-80">
            ...
            <input 
              type="text" 
              placeholder="Search sessions or customers..." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {sessions.map((session) => (
          <Card key={session.id} className="hover:shadow-md transition-shadow cursor-pointer border-slate-200 relative overflow-hidden flex flex-col" onClick={() => navigate(`/agent/session/${session.id}`)}>
            {session.risk === "high" && (
              <div className="absolute top-0 left-0 w-full h-1 bg-amber-500" />
            )}
            {session.status === "active" && session.risk !== "high" && (
              ...
              <div className="space-y-3 mt-4">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className={`font-medium capitalize ${
                    session.risk === "high" ? "text-amber-600" : 
                    session.risk === "medium" ? "text-blue-600" : "text-emerald-600"
                  }`}>
                    {session.risk}
                  </span>
                </div>
