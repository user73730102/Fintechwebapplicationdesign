        ...
        <div className="flex-1 flex flex-col gap-4 relative">
          <div className="flex-1 bg-slate-900 rounded-2xl overflow-hidden relative border border-slate-800 flex items-center justify-center shadow-2xl">
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
              ...
              {stage === 2 && <div className="bg-emerald-500/20 text-emerald-400 backdrop-blur-md px-3 py-1.5 rounded-lg text-sm font-medium border border-emerald-500/20 flex items-center gap-2">
            ...
            <div className="absolute bottom-8 right-8 w-28 h-28 bg-slate-900/95 backdrop-blur-xl rounded-3xl rounded-br-sm border-2 border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.4)] flex flex-col items-center justify-center">
              ...
              <div className="relative z-10 flex flex-col items-center gap-2">
                ...
                <div className="flex gap-1">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-bounce" style={{
                  animationDelay: "0ms"
                }} />
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-bounce" style={{
            ...
            <div className="absolute bottom-6 left-6 right-6 flex justify-between items-center">
              <Button variant={isMuted ? "destructive" : "secondary"} size="icon" className="rounded-full w-12 h-12 shadow-lg backdrop-blur-md bg-white/10 hover:bg-white/20 border border-white/10" onClick={() => setIsMuted(!isMuted)}>
                {isMuted ? <MicOff className="w-5 h-5 text-white" /> : <Mic className="w-5 h-5 text-white" />}
              </Button>

              <Button onClick={() => navigate("/onboarding/processing")} disabled={stage < 2} className={`px-6 gap-2 h-12 rounded-xl shadow-lg transition-all duration-500 ${stage === 2 ? "bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-500/20" : "bg-slate-800 text-slate-500 border border-slate-700 cursor-not-allowed opacity-50"}`}>
        ...
        <div className="w-full lg:w-[400px] bg-slate-900 rounded-2xl border border-slate-800 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map(msg => <div key={msg.id} className={`flex flex-col ${msg.sender === "user" ? "items-end" : "items-start"}`}>
                ...
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${msg.sender === "user" ? "bg-blue-600 text-white rounded-br-none" : "bg-slate-800 text-slate-100 rounded-bl-none border border-slate-700"}`}>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-900/50">
            <div className="flex items-center gap-2">
              <input type="text" placeholder="Type or speak your response..." className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSend()} />
