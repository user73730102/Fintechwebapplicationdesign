      ...
      <Card>
        <div className="p-4 border-b border-slate-100 flex gap-4 bg-slate-50/50">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search by name or ID..." className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {pastSessions.map(session => <tr key={session.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`flex items-center gap-1.5 ${session.risk === 'High' ? 'text-amber-600' : session.risk === 'Medium' ? 'text-blue-600' : 'text-emerald-600'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${session.risk === 'High' ? 'bg-amber-500' : session.risk === 'Medium' ? 'bg-blue-500' : 'bg-emerald-500'}`} />
                      {session.risk}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <Badge variant={session.status === 'Approved' ? 'success' : session.status === 'Rejected' ? 'destructive' : 'warning'}>
                      {session.status}
                    </Badge>
                  </td>
