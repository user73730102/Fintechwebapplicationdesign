...
export function CustomerProcessing() {
  ...
  const steps = [{
    ...
    detail: "Cross-referencing global databases"
  }, {
    id: 2,
    icon: Database,
    label: "Analyzing Financial Profile",
    detail: "Aggregating credit bureaus"
  }, {
    id: 3,
    icon: LineChart,
    label: "Assessing Risk Profile",
    detail: "Running AI fraud detection models"
  }, {
    id: 4,
    icon: FileCheck2,
    label: "Generating Tailored Offers",
    detail: "Finalizing loan options"
  }];
  useEffect(() => {
  ...
  return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-white">
      <div className="w-full max-w-lg space-y-12">
        ...
        <div className="space-y-6">
          {steps.map((step, index) => <div key={step.id} className="relative flex items-center gap-6">
              {/* Connecting line */}
              {index !== steps.length - 1 && <div className={`absolute left-6 top-14 w-0.5 h-12 -ml-px ${currentStep > index ? "bg-blue-500" : "bg-slate-800"}`} />}
              
              <div className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-xl border ${currentStep > index ? "bg-blue-600 border-blue-500 text-white shadow-[0_0_15px_rgba(37,99,235,0.5)]" : currentStep === index ? "bg-slate-800 border-blue-500 text-blue-400" : "bg-slate-900 border-slate-800 text-slate-600"} transition-all duration-500`}>
                <step.icon className="w-5 h-5" />
                
                {/* Ping animation for active step */}
                {currentStep === index && <span className="absolute flex h-full w-full">
              ...
              {currentStep > index && <motion.div initial={{
            scale: 0,
            opacity: 0
          }} animate={{
            scale: 1,
